...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        >>> expr.evaluate()
        False
        >>> expr = BoolOp('or', [Bool(True), Bool(True), Bool(True)])
        >>> expr.evaluate()
        True
        >>> expr = BoolOp('or', [Bool(False), Bool(False), Bool(False)])
        >>> expr.evaluate()
        False
        >>> expr = BoolOp('or', [Bool(False), Bool(True), Bool(True)])
        >>> expr.evaluate()
        True
        >>> expr = BoolOp('or', [Bool(False), Bool(False), Bool(True)])
        >>> expr.evaluate()
        True
        >>> expr = BoolOp('or', [Bool(False)])
        >>> expr.evaluate()
        False
        """
        if self.op == 'and':
            return all([i.evaluate() for i in self.values])
        elif self.op == 'or':
            return any([i.evaluate() for i in self.values])
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        for operator, subexpr in self.comparisons:
            if operator == '<=':
                if not self.left.evaluate() <= subexpr.evaluate():
                    return False
            elif operator == '<':
                if not self.left.evaluate() < subexpr.evaluate():
                    return False
