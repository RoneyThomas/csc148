...
class TMTree:
    ...
    *methods* for this interface.
    ...
    def update_rectangles(self, rect: Tuple[int, int, int, int]) -> None:
        ...
        sum_width = 0
        sum_height = 0
        last_finite_tree = None
        wid = 0
        hig = 0
        for i in range(len(self._subtrees)):
            if self._subtrees[i].data_size > 0:
                last_finite_tree = self._subtrees[i]
            if width > height and self.data_size > 0 and \
                    i != len(self._subtrees) - 1:
                if self._subtrees[i].data_size > 0:
                    wid = math.floor((self._subtrees[i].data_size /
                                      self.data_size) * width)
                    self._subtrees[i].update_rectangles((start_ver, y, wid,
                                                         height))
                    start_ver += wid
                    sum_width += wid
                i += 1
                    ...
                    i == len(self._subtrees) - 1:
                if self._subtrees[i].data_size > 0:
                    self._subtrees[i].update_rectangles((start_ver, y,
                                                         width - sum_width,
                                                         height))
                else:
                    start_ver -= wid
                    sum_width -= wid
                    last_finite_tree.update_rectangles((start_ver, y,
                                                        width - sum_width,
                                                        height))
            elif height >= width and self.data_size > 0 and \
                    i != len(self._subtrees) - 1:
                if self._subtrees[i].data_size > 0:
                    hig = math.floor((self._subtrees[i].data_size /
                                      self.data_size) * height)
                    self._subtrees[i].update_rectangles((x, start_hor, width,
                                                         hig))
                    start_hor += hig
                    sum_height += hig
                i += 1
                ...
                if self._subtrees[i].data_size > 0:
                    ...
                    start_hor -= hig
                    sum_height -= hig
                    last_finite_tree.update_rectangles((x, start_hor, width,
                                                        height - sum_height))
    ...
    def collapse_all(self) -> None:
        ...
        if self._parent_tree._parent_tree is None and \
