...
class TMTree:
    ...
    *methods* for this interface.
    ...
    def __init__(self, name: str, subtrees: List[TMTree],
        ...
        # settings for empty tree
        if self._subtrees:
            for child in self._subtrees:
                child._parent_tree = self
        ...
        try:
            if not self._subtrees:
                ...
                for subtree in self._subtrees:
                    assert subtree.data_size <= self.data_size
                    x , y, width, height = rect

                    # partition vertically

                    if width > height:
                        # manipluate x and width
                        if counter < n:
                            partition_ratio = subtree.data_size // self.data_size
                            subtree_width = width * partition_ratio
                            self.rect = (new_origin, y, subtree_width, height)
                            counter += 1
                            subtree.update_rectangles(
                                (new_origin, y + height, subtree_width, height))
                            new_origin += subtree_width

                        else:
                            # rightmost subtree of current tree
                            subtree_width = width - new_origin
                            self.rect = (new_origin, y, subtree_width, height)
                            assert width == new_origin + subtree_width
                            subtree.update_rectangles(
                                (new_origin, y + height, subtree_width, height))
                    else:
                        if counter < n:
                            # manipulate y and height
                            partition_ratio = subtree.data_size // self.data_size
                            subtree_height = height * partition_ratio
                            self.rect = (
                            x, new_origin, width, subtree_height)
                            subtree.update_rectangles(
                                (x, y + height, width, height))
                            new_origin += subtree_height
                            counter += 1
                        else:
                            # bottommost subtree on current tree
                            subtree_height = height - new_origin
                            self.rect = (x, new_origin, width, subtree_height)
                            assert height == new_origin+subtree_height
                            # subtree.update_rectangles(
                            #     (x, y + height, width, height))
    ...
    def get_rectangles(self) -> List[Tuple[Tuple[int, int, int, int],
        ...
        if not self._subtrees:
            ...
            assert len(self._colour) == 3
            return [(self.rect, self._colour)]
        else:
            temp = []
            for subtree in self._subtrees:
                temp += subtree.get_rectangles()
            # print('\n', temp)
            return temp
...
class FileSystemTree(TMTree):
    ...
    def __init__(self, path: str) -> None:
        ...
        if not os.path.isdir(path):
            ...
            print(self.data_size, self._name)
        else:
            super().__init__(os.path.basename(path), subtrees=[])
            for subtree in os.listdir(path):
                sub_path = os.path.join(path, subtree)
                node = FileSystemTree(sub_path)
                self._subtrees.append(node)
                # suppose to implement this in base initializer, but doesnt work
    ...
    def _is_leaf(self, path) -> bool:
        """
        leaf iff file or empty directory*(ask prof about this not sure)

        :param path:
        :return:
        """
        if not self._name and os.path.isdir(path):
            return True
