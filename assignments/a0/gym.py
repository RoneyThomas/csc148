...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        """
        # TODO: implement this method!
        for required_cert in workout_class.get_required_certificates():
            if required_cert not in self._certificates:
                return False
        return True


def _instructor_available(instr_id: int, time_point_items: []) -> bool:
    """Function to check if an instructor is available at a given time point"""
    for _, t in time_point_items:
        if t[0].get_id() == instr_id:
            return False
    return True
...
class Gym:
    ...
    def add_instructor(self, instructor: Instructor) -> bool:
        ...
        """
        # TODO: implement this method!
        if instructor.get_id() not in self._instructors.keys():
    ...
    def add_workout_class(self, workout_class: WorkoutClass) -> bool:
        ...
        """
        # TODO: implement this method!
        if workout_class.get_name() not in self._workouts.keys():
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        """
        # TODO: implement this method!
        # check if the time exist in schedule.
        if time_point in self._schedule.keys():
            # check if the room is available
            if room_name not in self._schedule[time_point].keys():
                # check if the instructor is available
                if self._instructors[instr_id].can_teach(
                        self._workouts[workout_name]) \
                        and _instructor_available(instr_id,
                                                  self._schedule[
                                                      time_point].items()):
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        if time_point not in self._schedule:
            return False
        # Checking if the client is registered for
        # any other class during that time
        for _, t in self._schedule[time_point].items():
            if client in t[2]:
                return False
        # Find the room of the workout and check
        # If there is space to register
        for r, t in self._schedule[time_point].items():
            if workout_name == t[1].get_name():
                if len(t[2]) < self._rooms[r]:
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        offerings = []
        if time_point in self._schedule.keys():
            for r, v in self._schedule[time_point].items():
                offerings.append((v[0].name, v[1].get_name(), r))
        return offerings
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        # TODO: implement this method!
        total_hours = {}
        for instructors_key in self._instructors:
            total_hours[instructors_key] = 0
        for d in self._schedule:
            if time1 <= d <= time2:
                for v in self._schedule[d].values():
                    total_hours[v[0].get_id()] += 1
        return total_hours
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        # TODO: implement this method!
        instructor_hours = self.instructor_hours(time1, time2)
        instructor_hours_id = sorted(instructor_hours.keys())
        payroll = []
        for i in instructor_hours_id:
            payroll.append((i,
                            self._instructors[i].name,
                            instructor_hours[i],
                            ((instructor_hours[i] * base_rate) +
                             self._instructors[
                                 i].get_num_certificates() * BONUS_RATE *
                             instructor_hours[i])))
        return payroll
