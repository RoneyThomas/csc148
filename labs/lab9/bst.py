...
class BinarySearchTree:
    ...
    def height(self) -> int:
        ...
        if self.is_empty():
            ...
            height = []
            height.append(self._left.height())
            height.append(self._right.height())
            return max(height) + 1
    ...
    def items_in_range(self, start: Any, end: Any) -> List:
        ...
        if self.is_empty():
            ...
            rtn_values = []
            if start <= self._root <= end:
                rtn_values.extend(self._left.items_in_range(start, end))
                rtn_values.append(self._root)
            if not self._right.is_empty():
                rtn_values.extend(self._right.items_in_range(start, end))
            return rtn_values
    ...
    def rotate_right(self) -> None:
        ...
        if not self._left.is_empty():
            new_root = BinarySearchTree(self._root)
            new_root._right = self._right
            new_root._left = self._left._right
            self._right = new_root
            self._root = self._left._root
            self._left._right = BinarySearchTree(None)
    ...
    def rotate_left(self) -> None:
        ...
        if not self._right.is_empty():
            new_root = BinarySearchTree(self._root)
            new_root._left = self._left
            new_root._right = self._right._left
            self._left = new_root
            self._root = self._right._root
            self._right._left = BinarySearchTree(None)
