...
def count_branch(t: Tree):
    c = 0
    for n in t._subtrees:
        c += len(n)
    return c


def count_node(t: Tree):
    c = 0
    for n in t._subtrees:
        if n._subtrees != []:
            c += 1
...
class Tree:
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty():
            ...
            branch_count = count_branch(self)
            node_count = 1 + count_node(self)
            return branch_count / node_count
    ...
    def insert(self, item: Any) -> None:
        ...
        if self.is_empty():
            ...
            if n == 3:
                ...
                x = random.randint(0, len(self._subtrees) - 1)
                self._subtrees[x].insert(item)
